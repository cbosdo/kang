...
class PersistedScheduler(sched.scheduler):
        ...
        if os.path.isfile(path):
            ...
            with open(self.path, 'r') as fd:
                ...
                for event in events:
                    ...
                    self.enterabs(float(event[0]), int(event[1]), func, argument=json.loads(event[3]), kwargs=json.loads(event[4]))
    ...
    def events(self):
        ...
        '''
        Event = namedtuple('Event', ['time', 'priority', 'action', 'argument', 'kwargs'])
        return [Event(event.time, event.priority, event.argument[0], event.argument[1], event.argument[2]) for event in self.queue]
    ...
    def save(self):
        ...
        with open(self.path, 'w') as fd:
            for event in self.events:
                ...
                kwargs = event.kwargs
                fd.write('{},{},{},{},{}\n'.format(event.time, event.priority, action.__name__, json.dumps(argument), json.dumps(kwargs)))
...
class SchedulerThread(threading.Thread):
    ...
    def cancel(self, time, action, argument=(), kwargs={}):
        ...
        for event in self.scheduler.events:
            if event.time == time and \
                    event.action == action and \
                    event.argument == argument and \
                    event.kwargs == kwargs:
                found = event
                break
        if not found:
